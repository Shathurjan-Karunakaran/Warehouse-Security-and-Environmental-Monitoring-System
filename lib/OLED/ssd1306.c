...
{
     // --- Direct I2C call ---
	 i2c_display_image(dev, page, seg, images, width); // Always use I2C

	 // Set to internal buffer
	 // Ensure seg is within bounds before memcpy
	 if (page >= 0 && page < dev->_pages && seg >= 0 && (seg + width) <= 128) {
		  memcpy(&dev->_page[page]._segs[seg], images, width);
	 } else {
		  // Optional: Log an error if bounds are exceeded
		  // ESP_LOGE("SSD1306", "Display image bounds error: page=%d, seg=%d, width=%d", page, seg, width);
	 }
}
...
void ssd1306_scroll_text(SSD1306_t * dev, char * text, int text_len, bool invert)
{
    ...
    // --- REMOVE or COMMENT OUT SPI Check ---
    /*
    void (*func)(SSD1306_t * dev, int page, int seg, uint8_t * images, int width);
    if (dev->_address == SPI_ADDRESS) {
        func = spi_display_image; // Remove
    } else {
        func = i2c_display_image; // Keep
    }
    */
    // --- REPLACE with direct function pointer assignment ---
    void (*func)(SSD1306_t * dev, int page, int seg, uint8_t * images, int width) = i2c_display_image; // Always use I2C
    ...
    while(1) {
        ...
        if(dstIndex >= 0 && dstIndex < dev->_pages && srcIndex >= 0 && srcIndex < dev->_pages) {
             for(int seg = 0; seg < dev->_width; seg++) {
                 dev->_page[dstIndex]._segs[seg] = dev->_page[srcIndex]._segs[seg];
             }
             (*func)(dev, dstIndex, 0, dev->_page[dstIndex]._segs, dev->_width); // Use dev->_width instead of sizeof
        } else {
             ESP_LOGW(__FUNCTION__, "Scroll index out of bounds: src=%d, dst=%d", srcIndex, dstIndex);
        }
    ...
    // Ensure srcIndex is valid before displaying text
    if(srcIndex >= 0 && srcIndex < dev->_pages) {
        int _text_len = text_len;
        if (_text_len > 16) _text_len = 16;
        ssd1306_display_text(dev, srcIndex, text, text_len, invert);
    } else {
         ESP_LOGW(__FUNCTION__, "Final scroll index invalid: src=%d", srcIndex);
    }
}
...
void ssd1306_wrap_arround(SSD1306_t * dev, ssd1306_scroll_type_t scroll, int start, int end, int8_t delay)
{
	...
	if (delay >= 0) {
		for (int page=0;page<dev->_pages;page++) {
			i2c_display_image(dev, page, 0, dev->_page[page]._segs, 128); // Always use I2C
...
void ssd1306_fadeout(SSD1306_t * dev)
{
    ...
    for(int page=0; page<dev->_pages; page++) {
        ...
        for(int line=0; line<8; line++) {
            ...
            for(int seg=0; seg<128; seg++) {
                // Check bounds before calling func
                if(page >= 0 && page < dev->_pages && seg >= 0 && seg < dev->_width) {
